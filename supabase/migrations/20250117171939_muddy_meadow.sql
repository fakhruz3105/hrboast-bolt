...
DROP POLICY IF EXISTS "inventory_images_select_policy_v3" ON storage.objects;
DROP POLICY IF EXISTS "inventory_images_insert_policy_v3" ON storage.objects;
DROP POLICY IF EXISTS "inventory_images_update_policy_v3" ON storage.objects;
DROP POLICY IF EXISTS "inventory_images_delete_policy_v3" ON storage.objects;

-- Create storage policies with company isolation
CREATE POLICY "inventory_images_select_policy_v4"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'inventory-images');

CREATE POLICY "inventory_images_insert_policy_v4"
...
CREATE POLICY "inventory_images_update_policy_v4"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'inventory-images');

CREATE POLICY "inventory_images_delete_policy_v4"
