...
-- Create the benefits table with simplified structure
...
-- Insert default benefits for Muslimtravelbug company
DO $$
DECLARE
  v_company_id uuid;
BEGIN
  ...
  IF v_company_id IS NOT NULL THEN
    ...
    INSERT INTO benefits (
      ...
      (v_company_id, 'Spectacles Claim', 'Annual reimbursement for prescription glasses or contact lenses', 500.00, true, 'Once per year'),
      (v_company_id, 'Birthday Rewards', 'Annual birthday celebration allowance', 100.00, true, 'Once per year'),
      (v_company_id, 'Medical Cards', 'Company medical card coverage for outpatient and inpatient care', 1500.00, true, 'Annual coverage'),
      (v_company_id, 'Medical Insurance', 'Comprehensive medical insurance coverage', 2000.00, true, 'Annual coverage'),
      (v_company_id, 'Baby Delivery', 'Maternity benefit for childbirth expenses', 3000.00, true, 'Once per child'),
      (v_company_id, 'Children Care', 'Monthly allowance for childcare expenses', 200.00, true, 'Monthly'),
      (v_company_id, 'Family Travel Scheme', 'Annual family vacation allowance', 2500.00, true, 'Once per year'),
      (v_company_id, 'Office Loan', 'Interest-free loan for office equipment or furniture', 5000.00, true, 'Once per employment'),
      (v_company_id, 'Passport Renewal', 'Reimbursement for passport renewal fees', 150.00, true, 'Every 5 years'),
      (v_company_id, 'Marry Me Incentive', 'One-time marriage allowance for staff', 1000.00, true, 'Once per employment');
