...
BEGIN
  ...
  -- Insert warning letter with explicit cast
  INSERT INTO warning_letters (
    ...
    p_staff_id,
    v_company_id,
    p_warning_level::text::warning_level,  -- Double cast to ensure proper conversion
    p_incident_date,
    p_description,
    p_improvement_plan,
    p_consequences,
...
BEGIN
  ...
  SELECT 
    wl.id,
    wl.staff_id,
    initcap(wl.warning_level::text),  -- Use initcap instead of UPPER
    wl.incident_date,
    wl.description,
    wl.improvement_plan,
    wl.consequences,
