...
-- Create simplified RLS policies for memos
...
CREATE POLICY "memos_insert"
  ON memos FOR INSERT
  WITH CHECK (true);  -- Allow all authenticated users to create memos

CREATE POLICY "memos_delete"
  ON memos FOR DELETE
  USING (true);  -- Allow all authenticated users to delete memos

-- Create function to get memos for a staff member
CREATE OR REPLACE FUNCTION get_staff_memos(p_staff_id uuid)
RETURNS TABLE (
  id uuid,
  title text,
  type memo_type,
